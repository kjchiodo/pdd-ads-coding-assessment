...
# Output Files: ds.rds, ds.csv, q1_log.txt
...
tryCatch({
  ...
  # Mapping Unscheduled visits in VISITNUM
  ds <- ds %>%
    mutate(
      VISITNUM_chr = case_when(
        str_detect(toupper(VISIT), "AMBUL ECG REMOVAL") ~ "6.0",
        str_detect(toupper(VISIT), "UNSCHEDULED") ~ str_extract(VISIT, "\\d+(\\.\\d+)?"),
        TRUE ~ as.character(VISITNUM)
      )
    ) %>%
    mutate(
      VISITNUM = as.numeric(VISITNUM_chr)
    ) %>%
    select(-VISITNUM_chr)
